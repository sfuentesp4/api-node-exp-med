...
import { CatalogoEstadosCitaService } from "../services/CatalogoEstadosCitaService";

export class CatalogoEstadosCitaController {
  static async obtenerTodos(req: Request, res: Response): Promise<void> {
    try {
      const estados = await CatalogoEstadosCitaService.obtenerTodos();
  ...
  static async crearEstado(req: Request, res: Response): Promise<void> {
    try {
      const nuevoEstado = await CatalogoEstadosCitaService.crearEstado(req.body);
  ...
  static async obtenerPorId(req: Request, res: Response): Promise<void> {
    try {
      const estado = await CatalogoEstadosCitaService.obtenerPorId(Number(req.params.id));
      if (!estado) {
  ...
  static async actualizarEstado(req: Request, res: Response): Promise<void> {
    try {
      const estadoActualizado = await CatalogoEstadosCitaService.actualizarEstado(
  ...
  static async eliminarEstado(req: Request, res: Response): Promise<void> {
    try {
      const resultado = await CatalogoEstadosCitaService.eliminarEstado(Number(req.params.id));
      if (!resultado) {
        res.status(404).json({ mensaje: "Estado no encontrado" });
        return;
      }
