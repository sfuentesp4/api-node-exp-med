...
import { CatalogoEstadosFacturaService } from "../services/CatalogoEstadosFacturaService";

export class CatalogoEstadosFacturaController {
  static async obtenerTodos(req: Request, res: Response): Promise<void> {
    try {
      const estados = await CatalogoEstadosFacturaService.obtenerTodos();
  ...
  static async crearEstado(req: Request, res: Response): Promise<void> {
    try {
      const nuevoEstado = await CatalogoEstadosFacturaService.crearEstado(req.body);
  ...
  static async obtenerPorId(req: Request, res: Response): Promise<void> {
    try {
      const estado = await CatalogoEstadosFacturaService.obtenerPorId(Number(req.params.id));
      if (!estado) {
  ...
  static async actualizarEstado(req: Request, res: Response): Promise<void> {
    try {
      const estadoActualizado = await CatalogoEstadosFacturaService.actualizarEstado(
  ...
  static async eliminarEstado(req: Request, res: Response): Promise<void> {
    try {
      const resultado = await CatalogoEstadosFacturaService.eliminarEstado(Number(req.params.id));
      if (!resultado) {
        res.status(404).json({ mensaje: "Estado no encontrado" });
        return;
      }
